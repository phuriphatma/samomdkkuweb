...
| **Dev account** PAT + `SUPABASE_DEV_*` block | ❌ not in git, ✅ **shareable with the team** | `.env.local` only. Deliberately different from every row above: the `samo-dev` account (`samomdkkuaiorg`) holds **nothing but disposable projects**, which is the whole reason `docs/TEAM-WORKFLOW.md` D7 chose a separate account. `SUPABASE_DEV_URL` · `_ANON_KEY` · `_ACCESS_TOKEN` · `_DB_URL`. ⚠️ **dev holds REAL production data** (D1 — no masking), so the URL is still not published |
| **Dev** Google OAuth client (`GOOGLE_DEV_CLIENT_ID` / `GOOGLE_DEV_CLIENT_SECRET`) | ❌ not in git, ✅ shareable with the team | `.env.local` only — added 2026-08-31 so **previews can use Google sign-in** (`npm run dev:google`; steps in `docs/TEAM-WORKFLOW.md` §3a). Same tier as the `SUPABASE_DEV_*` block: it authenticates against **`samo-dev` only**, so its worst case is impersonating the PREVIEW sign-in flow. ⛔ **Production has its own, separate client and this must never become it** — dev credentials are deliberately shared with the whole team, so production's secret here would let any of them impersonate the real site's Google sign-in. Reset at console.cloud.google.com → Credentials → that client → Reset secret, then re-run `npm run dev:google` |
