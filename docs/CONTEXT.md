...
Presence. Remote edits merge into the in-memory model and re-render (debounced;
deferred during a drag). Last-write-wins, not OT. The socket is re-authed every
20 min because `db` runs with autoRefreshToken off. `currentAccessToken()` is
exported from `db.js` for `realtime.setAuth()`.
