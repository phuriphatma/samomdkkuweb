# 2026-08-27 — the session narrative pruned out of STATE.md

Every dated block below was in `STATE.md`'s `## NEXT-SESSION PROMPT` until the
split (`docs/TEAM-WORKFLOW.md` §6.5). Nothing was rewritten — the blocks are
verbatim, in the order they were in, so `git log --oneline` and this file agree.

**What was NOT moved here:** durable rules went to `docs/INVARIANTS.md`, and
what is true right now stayed in `STATE.md`. If a block below contradicts either
of those, THEY are current and this is history.
...
# Appendix A — the per-deploy verification log, verbatim

`STATE.md`'s `## CURRENT DEPLOY` had grown to 293 lines of "which string was
grepped in which served bundle, and which one read 0 and looked like a failed
deploy". Every one of those is EVIDENCE about a deploy that has happened, so it
belongs in an archive rather than in a status file — but the traps in it are
live, and the ones that generalise were lifted into `STATE.md`'s new deploy
block and `docs/INVARIANTS.md`.

⚠️ **The head of this block said `2993dd1` while the NEXT-SESSION PROMPT said
`36ac1d5`, which is one deploy later.** That contradiction is why the new
`STATE.md` keeps the deployed sha in exactly one place.
...
# Appendix B — the "STATE.md is 1,126 lines" section and the old prompt preamble

Superseded by the split itself. Kept because the preamble carried the rule the
split was built on, and because a section complaining about a problem is worth
reading beside the commit that fixed it.
