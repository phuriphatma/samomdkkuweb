...
Username pattern `samomdkku<short>` / password `«pattern retired»`:
...
`uni_staff` (sastaff / weak pw — «redacted, still live 2026-08-17»). Both are seat-style, mirroring
...
2. Supabase Dashboard → Authentication → Add user:
   ...
   - `sastaff@samomdkku.app` with a weak password «redacted»
   Then run:
   ```sql
