# Codex / Agent Router — samomdkkuweb
...
- `.Codex/rules/mistakes.md` — hard-learned anti-patterns (READ before touching auth/network code)
- `.Codex/rules/security.md` — API key hygiene
- `skills/*.md` — playbooks for the non-obvious workflows

When working in `src/js/auth.js` or `src/js/db.js` — ALWAYS read
`.Codex/rules/mistakes.md` first. Those modules carry sharp edges.
...
2. **If a new bug class was discovered**: append to `.Codex/rules/mistakes.md`.
