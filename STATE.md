...
| 0031_project_doc_views.sql | Per-user, per-doc seenAt marker — moves the inbox "I've seen this หนังสือ" state off per-device localStorage into a server-side table so highlights sync across devices AND don't leak across accounts on the same browser. RLS: each user only reads/writes their own rows. Pre-apply, inbox falls back to the old localStorage behaviour and warns once. JS bulk-uploads each device's existing localStorage seenAt to this table on first run after the upgrade, so "new device opens to many phantom notifications" goes away. | ❌ pending |
